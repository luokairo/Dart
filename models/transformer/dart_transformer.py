"""This file contains code for Dart Transformer.

This file is adopted and modified from https://github.com/mit-han-lab/hart/blob/main/hart/modules/models/transformer/hart_transformer_t2i.py
"""
...
from dart.models.networks.basic_hart import (
    AdaLNBeforeHead,
    AdaLNSelfAttn,
    LlamaRMSNormFused,
    TimestepEmbedder,
...
from dart.models.networks.utils import (
    gumbel_softmax_with_rng,
    sample_with_top_k_top_p_,
)
from dart.utils import get_device
...
class DARTForT2I(PreTrainedModel):
    ...
    def __init__(self, vae: DARTAutoEncoder, config: DARTForT2IConfig):
        ...
        quant: DartHybridQuantizer = vae_local.quantize
        self.vae_proxy: Tuple[DARTAutoEncoder] = (vae_local,)
        self.vae_quant_proxy: Tuple[DartHybridQuantizer] = (quant,)
        ...
        # Jingyi: using a mlp to make continuous tokens
        self.continous_embed = nn.Sequential(      
            ...
            nn.Linear(self.C, self.C),
            nn.GELU(),
            nn.Linear(self.C, self.Cvae)
        )
        self.image_embed = nn.Linear(self.V, self.Cvae)
    ...
    def get_continous_logits(
        ...
        if not isinstance(h_or_h_and_residual, torch.Tensor):
            ...
            h = h_or_h_and_residual
        return self.continous_embed(self.head_nm(h.float(), cond_BD).float()).float()
    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums[:-1]):
            ...
            x = next_token_map # 2*B, l, C
            AdaLNSelfAttn.forward
            for b in self.blocks:
                # si used for position embed
                x = b(
                    x=x,
            ...
            continuous_logits_BlV = (1 + t) * continuous_logits_BlV[:B] - t * continuous_logits_BlV[B:] # B, l, Cvae
